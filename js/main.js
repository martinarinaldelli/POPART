document.addEventListener('DOMContentLoaded', function () {
    ...
    const swiper = new Swiper(".color-palette-carousel", {
    ...
    // Balloon Gallery Modal
    if (document.querySelector('.balloon-gallery')) {
        const modal = document.getElementById('balloon-modal');
        const balloons = document.querySelectorAll('.balloon');
        const closeButton = modal.querySelector('.close-balloon-modal');
        const prevButton = modal.querySelector('.prev-balloon');
        const nextButton = modal.querySelector('.next-balloon');
        const modalImage = modal.querySelector('.balloon-modal-image img');
        const modalTitle = modal.querySelector('.balloon-modal-title');
        const modalDescription = modal.querySelector('.balloon-modal-description');
        let currentIndex = 0;

        const balloonData = Array.from(balloons).map(balloon => ({
            src: balloon.querySelector('img').src,
            title: balloon.dataset.title,
            description: balloon.dataset.description
        }));
        ...
        const updateModalContent = () => {
            const slide = balloonData[currentIndex];
            modalImage.src = slide.src;
            modalTitle.textContent = slide.title;
            modalDescription.textContent = slide.description;
        };
        ...
        const showNext = () => {
            currentIndex = (currentIndex + 1) % balloonData.length;
            updateModalContent();
        };

        const showPrev = () => {
            currentIndex = (currentIndex - 1 + balloonData.length) % balloonData.length;
            updateModalContent();
        };

        balloons.forEach((balloon, index) => {
            balloon.addEventListener('click', () => openModal(index));
        });
